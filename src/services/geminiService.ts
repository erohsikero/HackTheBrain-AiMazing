...
export const analyzeSymptoms = async (symptoms: string): Promise<TriageResult> => {
  ...
  try {
    ...
    const prompt = `
    You are a dental triage AI assistant for MH2 Dental Clinic. Analyze the following patient symptoms and provide a JSON response with triage information.
...
export const getChatbotResponse = async (userMessage: string): Promise<string> => {
  ...
  try {
    ...
    const prompt = `
    You are enamAI, a helpful dental assistant chatbot for MH2 Dental Clinic. 
    ...
    IMPORTANT: Use only plain text in your response. Do not use any markdown formatting like bold, italic, bullet points, or special characters for emphasis.
    `;
...
const getFallbackChatResponse = (userMessage: string): string => {
  ...
  return "I'm here to help with questions about MH2 Dental Clinic! You can ask me about our services, pricing, hours, insurance, or booking appointments. How can I assist you today?";
};